...
#modo debug 
IS_DEBUG = False  # Desactivamos el modo debug para ocultar las hitboxes
...
PLAYER_SPEED = 4
ENEMY_MAX_HEALTH = 3
ENEMY_SPEED = 1.5  # Aumentamos la velocidad base de los enemigos
...
POWERUP_SIZE = (32, 32)
POWERUP_SCORE_THRESHOLDS = [20, 40, 60]  # Puntuaciones para que aparezcan los power-ups
POWERUP_DAMAGE = [1.5, 2.0, 2.5]  # Daño correspondiente a cada power-up
...
BOSS_SPEED = 2
BOSS_DASH_SPEED = 8
BOSS_SCORE_THRESHOLD = 60  # Puntaje necesario para que aparezca el jefe

# Configuración del logro
ACHIEVEMENT_SIZE = (200, 200)  # Tamaño del logro en pantalla
ACHIEVEMENT_ASSET = "../assets/achievement.png"  # Asset del logro
VICTORY_TEXT = "¡Has derrotado al jefe!"
...
is_game_over = False
powerups = []  # Lista global para los power-ups
is_victory = False  # Variable para controlar el estado de victoria
...
class Player(Entity):
    ...
    def __init__(self, x, y, width, height, tileset, speed):
        ...
        self.current_damage = 1  # Daño actual del jugador
        self.current_bullet_asset = BULLET  # Asset actual de la bala
...
class Boss(Entity):
    def __init__(self, x, y, width, height, tileset, speed):
        ...
        self.m_width = width
        self.m_height = height
        self.width = width  # Cambiamos para que empiece con tamaño completo
        self.height = height
        self.grow_speed = 4
        ...
        # Hitboxes más grandes y precisas
        self.hitboxes = {
    ...
    def destroy(self):
        objects.remove(self)
        global boss, is_victory
        boss = None
        is_victory = True  # Activar estado de victoria
...
class PowerUp(Object):
    def __init__(self, x, y, damage, bullet_image):
        super().__init__(x, y, POWERUP_SIZE[0], POWERUP_SIZE[1], bullet_image)
        self.collider = [self.width, self.height]
        self.damage = damage
        self.float_offset = 0
        self.float_speed = 0.1
        self.bullet_asset_path = bullet_image  # Guardamos el path del asset
        
    def update(self):
        # Efecto de flotación similar al corazón
        self.float_offset = math.sin(pygame.time.get_ticks() * self.float_speed) * 5
        self.draw()
...
class Achievement(Object):
    def __init__(self):
        ...
        x = WINDOW_SIZE[0] / 2 - ACHIEVEMENT_SIZE[0] / 2
        y = WINDOW_SIZE[1] / 2 - ACHIEVEMENT_SIZE[1] / 2
        super().__init__(x, y, ACHIEVEMENT_SIZE[0], ACHIEVEMENT_SIZE[1], ACHIEVEMENT_ASSET)
        self.float_offset = 0
        self.float_speed = 0.05  # Más lento que los otros efectos de flotación
    ...
    def draw(self):
        ...
        # Dibujar el texto de victoria
        victory_text = text_font.render(VICTORY_TEXT, True, BLACK)
        text_x = WINDOW_SIZE[0] / 2 - victory_text.get_width() / 2
        text_y = self.y + self.height + 20  # 20 píxeles debajo del logro
        WINDOW.blit(victory_text, (text_x, text_y))
...
def start():
    ...
    player.current_damage = 1  # Reiniciar daño
    player.current_bullet_asset = BULLET  # Reiniciar bullet asset
...
def shoot():
    ...
    bullet.damage = player.current_damage  # Asignamos el daño actual del jugador
...
def restart():
    ...
    global player, enemies, bullets, particles, objects, score, is_game_over, boss, hearts, powerups, is_victory    
    ...
    load_high_score()
    is_game_over = False
    is_victory = False  # Reiniciar estado de victoria
...
def enemy_spawner():
    ...
    # Si estamos en estado de victoria, no generamos enemigos
    if is_victory:
        return
    # Si el score llega a 60 y no hay jefe, creamos el jefe
...
def check_powerup_spawn():
    global score
    # Verificar si el score actual coincide con algún umbral
    if score in POWERUP_SCORE_THRESHOLDS and len(powerups) == 0:
        # Obtener el índice del power-up actual
        power_index = POWERUP_SCORE_THRESHOLDS.index(score)
        # Posición aleatoria para el power-up
        randomX = random.randint(BOUNDS_X[0], BOUNDS_X[1] - POWERUP_SIZE[0])
        randomY = random.randint(BOUNDS_Y[0], BOUNDS_Y[1] - POWERUP_SIZE[1])
        # Crear el power-up con el daño correspondiente
        powerup = PowerUp(randomX, randomY, POWERUP_DAMAGE[power_index], f"../assets/powerup{power_index + 1}.png")
        powerups.append(powerup)
        # Efecto visual de aparición
        spawn_particles(randomX, randomY)
        return True
    return False
...
while True:
    ...
    for powerup in powerups[:]:  # Usamos una copia de la lista para evitar problemas al modificarla
        if check_collisions(powerup, player):
            # Actualizar daño y asset del jugador
            player.current_damage = powerup.damage
            player.current_bullet_asset = powerup.bullet_asset_path
            # Efecto visual de recolección
            spawn_particles(powerup.x, powerup.y)
            offset = screen_shake(3, 4)
            # Eliminar el power-up
            objects.remove(powerup)
            powerups.remove(powerup)

    # Verificar si debemos generar un power-up
    check_powerup_spawn()
    ...
    if boss is not None:
        ...
        # Colisión jefe/balas
        for b in bullets[:]:  # Usamos una copia de la lista para evitar problemas al modificarla
            if boss is not None and boss.check_hit(b):  # Verificación adicional de seguridad
                bullets.remove(b)
                objects.remove(b)
